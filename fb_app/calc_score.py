...
def calc_score(league, winner_list):
    '''takes a user and optional list of winners.  If no list provided, goes to
    the web to get the winners.  updates WeekScore objects and returns an actual
     scores and projected scores list'''
    ...
    for player in Player.objects.filter(league=league).order_by('name'):
        ...
        #gets picks for any player with picks (for display only, do i need here?)
        while pick_num > 0:
        ...
        with urllib.request.urlopen(json_url) as field_json_url:
            ...
            #not needed for projection, uncomment if using for normal
#            try:
              #for game in Games.objects.filter(week=0):
#              for game in Games.objects.filter(week=week).exclude(qtr='Final'):
...
#                pass
            ...
            for player in Player.objects.filter(league=league):
                ...
                for pick in Picks.objects.filter(player=player, week=week):
                    ...
                    if game.qtr == "Final":
                        ...
                        elif pick.team.nfl_abbr not in winner_list:
                            print (player, pick.pick_num, str(pick.team))
                            score += pick.pick_num
                    else:
            ...
            return (scores_list, projected_scores_list)
