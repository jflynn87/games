...
function updateFedEx() {
      ...
      $('#update_fedex_setup_status').text('Skipping')      
      resolve()
            //})
...
return new Promise(function (resolve,reject) {
      ...
      $('#setup_table tbody').append('<tr id=field_update><td>Updating All Field stats</td><td id=field_update_status>Updating <span class=status>....</span></td>')                    
      ...
      fetch("/golf_app/field_updates/", 
            ...
            .then((response) => {
                  if (response.ok) {
                        console.log('field updates response ok', response.status)
                        return response.json()
                        }
                  else
                        {console.log('updateGolferStats error: ', response)
                        $('#field_update_status').text('Error ' + response.status + ':' + response.statusText)}
                        throw new Error('field updates error: ' + response.status + ':' + response.statusText)      
            ...
            .then((responseJSON) => {
                  ...
                  $('#field_update_status').text('Complete')                    
                  resolve()    

            })
            })
            // .then((response) => {
            //       console.log('field updates done, resolving ')
            //       //resolve()
            //             })
            //             console.log('all field updates done, resolving')
            //             $('#setup_table tbody').append('<tr id=field_update><td>Updating All Field stats</td>Complete</td>')                    
            //             resolve()    
            //       }
            //)
            //})
