...
var fn=function wrapper(g){return new Promise(resolve=>resolve(build_field(g,info,picks)));};var actions=groups.map(fn)
...
checkStarted()}))})})
function build_field(g,info,pick_data){return new Promise(function(resolve,reject){$('#field_sect #pick_form').append('<table id=tbl-group-'+g+' class=table> \
                                <thead class=total_score> <th> Group: '+g+'</th> </thead>'+'</table>')
const frag=new DocumentFragment()
console.log('pick array ',pick_data)
for(var p in Object.values(pick_data)){console.log('P ',p)
console.log('pick id: ',p.id)}
...
get_info(info,this);});golfer.appendChild(inputB)
