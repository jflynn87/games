...
inputB.addEventListener('change',function(evt){console.log('thsi',this)
$('#pick-status').empty()
get_info(info,this)})
...
resolve()}
document.getElementById('tbl-group-'+g).appendChild(frag);$('input#'+field.id).on('change',function(evt){$('#pick-status').empty()
get_info(info,this)})})})})}
