...
if(field.withdrawn){const status='WD'
...
inputB.addEventListener('change',function(){console.log('click happened!');});inputB.onchange=function(){console.log('click happened! 12');};field_row.classList.add('top_row')
golfer.appendChild(inputB)}
else if(field.started){const status='Started'
field_row.classList.add('started')}
else{const status='Problem'
field_row.classList.add('started')}
...
text=document.createTextNode(status+field.playerName+' ')
golfer.append(text)
