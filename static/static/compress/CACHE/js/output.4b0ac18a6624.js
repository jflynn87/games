...
Promise.all([fetch("/golf_app/get_info/"+$('#tournament_key').text()).then(response=>response.json()),fetch("/golf_app/field_get_picks").then(response=>response.json())]).then((responseJSON)=>{data=responseJSON
info=$.parseJSON(data[0])
picks=$.parseJSON(data[1])
...
else{$('input[name=group-6'+']').parent().parent().prop('hidden',true)
$('input[name=group-6'+']:checked').parent().parent().prop('hidden',false)
