...
Promise.all([fetch("/golf_app/get_info/"+$('#tournament_key').text()).then(response=>response.json()),fetch("/golf_app/field_get_picks").then(response=>response.json())]).then((responseJSON)=>{info=$.parseJSON(responseJSON[0])
picks=$.parseJSON(responseJSON[1])
var pick_array=[];for(let i=0;i<picks.length;i++){pick_array.push(picks[i].playerName.id)}
buildHeader()
groups=[]
$.each($('#groups_list li'),function(i,g){groups.push(g.innerText)})
var fn=function wrapper(g){return new Promise(resolve=>resolve(build_field(g,info,pick_array)));};var actions=groups.map(fn)
var results=Promise.all(actions)
results.then((()=>{$('[data-toggle="tooltip"]').tooltip();console.log('getting field js')
$.getScript('/static/golf_app/js/field.js')
$('#field_sect form').append('<div id=bottom_sect class=field_stats_display></div>')
$('#bottom_sect').append('<div id=bottom class=field_stats_display>'+'<div id=stats-dtl-toggle>'+'<h5>Show Stats <i class="fa fa-plus-circle show" style="color:lightblue"></i></h5>'+'</div>'+'<div id=pick-status></div>'+'<div id=grp_6_buttons class=grp_6_buttons>'+'<p id=jump_to style="background-color:lightgray; color:white; font-weight:bold;"> Jump to: '+'</p>'+'</div>'+'<input id=sub_button type="submit" class="btn btn-secondary" value="Submit Picks" disabled>')
...
checkStarted()}))})})
function build_field(g,info,pick_array){return new Promise(function(resolve,reject){$('#field_sect #pick_form').append('<table id=tbl-group-'+g+' class=table> \
                                <thead class=total_score> <th> Group: '+g+'<i id=expand-grp-'+g+' class="fa fa-plus-circle show" style="color:white; float:right;"> Show Group Stats</i> </th> </thead>'+'</table>')
$('#expand-grp-'+g).on('click',function(){toggle_stats_display(this,$('#tbl-group-'+g))})
...
$('#top_sect').append('<br> <div id=stats-dtl-toggle>'+'<h5>Show Stats <i class="fa fa-plus-circle show" style="color:lightblue"></i></h5>'+'<br></div>')
...
function build_golfer_row(field,pick_array){if(field.id=='18919'){field.started=false}
if(field.id=='18918'){field.withdrawn=true}
...
var status=' '
if(field.withdrawn){var status='WD'+' '
...
inputB.addEventListener('change',function(evt){$('#pick-status').empty()
get_info(info,this);});if(pick_array.indexOf(field.id)!=-1){inputB.checked=true}
if(field.lock_group){inputB.disabled=true}
golfer.appendChild(inputB)
golfer_row.classList.add('top_row')}
else if(field.started){let inputA=document.createElement('input');inputA.type='hidden';inputA.name="csrfmiddlewaretoken";inputA.value=$.cookie('csrftoken')
golfer.appendChild(inputA)
let inputB=document.createElement('input');inputB.id=field.id
inputB.type=input_type
inputB.classList.add(input_class)
...
inputB.name="group-"+field.group.number
inputB.value=field.id
if(pick_array.indexOf(field.id)!=-1){inputB.checked=true}
golfer.appendChild(inputB)
var status='Started'+' '
golfer_row.classList.add('started')}
else{var status='Problem'+' '
golfer_row.classList.add('started')}
...
if(field.started||field.withdrawn){t=field.playerName.strike()+' '+status}
else{t=field.playerName+' '+status}
...
return golfer_row}
function build_stats_row(field){let stats_row=document.createElement('tr');stats_row.id='stats_row-'+field.id
...
let stats_table=document.createElement('table');stats_table.style.width='100%'
...
for(let i=0;i<rec_l;i++){var r=Object.values(field.recent)[i]
...
return stats_row}
function toggle_stats_display(ele,table){t_id=table[0].id
var toggle_ind=document.getElementById(ele.id).innerHTML.trim()
if(toggle_ind=='Show Group Stats'){$('#'+t_id+' .stats_row').attr('hidden',false)
...
for(let i=0;i<expand_l;i++){table[0].getElementsByClassName('expand')[i].classList.remove('fa-plus-circle')
...
expand_l=table[0].getElementsByClassName('expand').length
for(let i=0;i<expand_l;i++){table[0].getElementsByClassName('expand')[i].classList.remove('fa-minus-circle')
...
function toggle_golfer_stats(field_id){indicator=document.getElementById('expand-'+field_id).innerHTML.trim()
if(indicator=="Show Golfer Stats"){$('#stats_row-'+field_id).removeAttr('hidden')
...
else{$('#stats_row-'+field_id).attr('hidden',true)
